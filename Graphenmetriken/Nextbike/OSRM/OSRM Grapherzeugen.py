...
# --- Daten laden ---
df = pd.read_csv("df_nextbike_merged_mit_routenOSRM.csv")  #Hier momentan die Daten von Radek enthalten, also OSRM
...
cache_file = Path("OSRMedge_matching_cache.csv")
...
sp.save_npz("adjacency_matrix.npz", adj_matrix)
print("Sparse Matrix gespeichert als: adjacency_matrix.npz")

# Node-Index speichern (um später OSM-IDs zuzuordnen)
pd.DataFrame({"osm_id": nodes, "matrix_index": range(n)}).to_csv("OSRMnode_index.csv", index=False)
print("Node-Index gespeichert als: OSRMnode_index.csv")

# Optional: Als dense CSV (nur bei kleinen Graphen sinnvoll!)
if n <= 5000:
    dense = pd.DataFrame(adj_matrix.toarray(), index=nodes, columns=nodes)
    dense.to_csv("OSRMadjacency_matrix_dense.csv")
    print("Dense Matrix gespeichert als: OSRMadjacency_matrix_dense.csv")
else:
