...
gdf = gpd.read_file("stadtradeln2022_osrm_routes.geojson")
...
cache_file = Path("edge_matching_cache_Stadtradeln2022.csv")
...
output_file = "mannheim_Stadtradeln2022_heatmapOSMR.png"
...
# --- Speichern ---
sp.save_npz("adjacency_matrixStadtradeln2022.npz", adj_matrix)
print("Sparse Matrix gespeichert als: adjacency_matrixStadtradeln2022.npz")

pd.DataFrame({"osm_id": nodes, "matrix_index": range(n)}).to_csv("node_indexStadtradeln2022.csv", index=False)
print("Node-Index gespeichert als: node_indexStadtradeln2022.csv")

if n <= 5000:
    dense = pd.DataFrame(adj_matrix.toarray(), index=nodes, columns=nodes)
    dense.to_csv("adjacency_matrix_denseStadtradeln2022.csv")
    print("Dense Matrix gespeichert als: adjacency_matrix_denseStadtradeln2022.csv")
else:
