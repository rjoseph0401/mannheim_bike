...
gdf = gpd.read_file("stadtradeln2024_osrm_routes.geojson")
...
cache_file = Path("edge_matching_cache_Stadtradeln2024.csv")
...
# Farbskala: Gelb-Grün = wenig, Rot/Pink = viel
base_cmap = plt.get_cmap("turbo")
cmap_log = mcolors.LinearSegmentedColormap.from_list(
    "turbo_trimmed",
    base_cmap(np.linspace(0.15, 1.0, 512))
).reversed()

# ── hits aus dem bereits berechneten Dict übernehmen ─────────────────────────
# 'hits' und 'nearest'/'route_ids' wurden weiter oben im Skript befüllt
hit_values = np.array(list(hits.values()))
vmin = max(1, np.percentile(hit_values, 5))
vmax = np.percentile(hit_values, 98)
norm_log = LogNorm(vmin=vmin, vmax=vmax)

# Kanten-Geometrien des großen Graphen
edges_large_gdf = ox.graph_to_gdfs(G, nodes=False, edges=True)
edge_geom_dict  = edges_large_gdf["geometry"].to_dict()

# Segmente sortiert aufbauen (wenig zuerst → viel oben)
seg_list = sorted(hits.items(), key=lambda x: x[1])

segments, seg_colors, seg_widths = [], [], []
for (u, v, k), count in seg_list:
    geom = edge_geom_dict.get((u, v, k))
    if geom is None:
        continue
    line_list = [geom] if geom.geom_type == "LineString" else list(geom.geoms)
    n_val = norm_log(max(count, vmin))
    color = cmap_log(n_val)
    lw    = 0.5 + 4.0 * n_val
    for line in line_list:
        segments.append(np.array(line.coords))
        seg_colors.append(color)
        seg_widths.append(lw)

lc_log = LineCollection(segments, colors=seg_colors, linewidths=seg_widths,
                        alpha=0.85, zorder=2)

# ── Abbildung ─────────────────────────────────────────────────────────────────
fig2, ax2 = plt.subplots(figsize=(14, 14), facecolor="white")
ax2.set_facecolor("white")
ax2.set_aspect("equal")

edges_large_gdf.plot(ax=ax2, color="#cccccc", linewidth=0.2, zorder=1)
ax2.add_collection(lc_log)
ax2.autoscale_view()
...
ax2.set_xticks([])
ax2.set_yticks([])
ax2.set_title("Stadtradeln 2024 – Routen-Heatmap (logarithmisch, ungeclippt)",
              fontsize=13, pad=12)

# ── Colorbar ──────────────────────────────────────────────────────────────────
sm2 = ScalarMappable(cmap=cmap_log, norm=norm_log)
sm2.set_array([])
cbar_ax2 = fig2.add_axes([0.88, 0.15, 0.025, 0.65])
cbar2 = fig2.colorbar(sm2, cax=cbar_ax2)
cbar2.set_label("Anzahl gematchter Routen (log)", fontsize=9, labelpad=8)

nice_ticks = [t for t in [1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000]
              if vmin <= t <= vmax]
...
# ── Speichern ─────────────────────────────────────────────────────────────────
OUT2 = Path("stadtradeln2024_heatmap_log_ungeclippt.png")
...
output_file = "mannheim_nextbike_heatmapOSMR.png"
...
# --- Speichern ---
sp.save_npz("adjacency_matrixStadtradeln2024.npz", adj_matrix)
print("Sparse Matrix gespeichert als: adjacency_matrixStadtradeln2024.npz")

pd.DataFrame({"osm_id": nodes, "matrix_index": range(n)}).to_csv("node_indexStadtradeln2024.csv", index=False)
print("Node-Index gespeichert als: node_indexStadtradeln2024.csv")

if n <= 5000:
    dense = pd.DataFrame(adj_matrix.toarray(), index=nodes, columns=nodes)
    dense.to_csv("adjacency_matrix_denseStadtradeln2024.csv")
    print("Dense Matrix gespeichert als: adjacency_matrix_denseStadtradeln2024.csv")
else:
